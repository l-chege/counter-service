...
# define path for the counter file to store the data in docker volume
...
def handle_request():
    ...
    if request.method == 'POST':
        ...
        # For GET requests, just return the current count value
        return f"Current POST requests count: {counter}"
...
def health_check():
    ...
    Returns:
        tuple: A tuple of JSON response indicating the health status and the HTTP status code.
    """
    try:
        ...
        # Return an unhealthy status if any error occurs, e.g., filw access issues.
        return jsonify({'status': 'unhealthy', 'reason': str(e)}), 500
    
    if __name__ == '__main__':
        # Run the Flask app with binding to all interfaces on port 8080.
        # Debug mode is diasbled for production use.
